...
class ModbusLabels:
    ...
    def setup_initialization(self):
        ...
        if self.connect_manager.setup_client:
            ...
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_sliding_reference_voltage_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_sliding_reference_voltage_type.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_sliding_reference_voltage_setup_access.value, 1)
            ...
            self.connect_manager.setup_client.write_register(ecm.addr_measurement_setup_access.value, 1)
            ...
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_rstp_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_rstp.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_rstp_setup_access.value, 1)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_storm_control_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_storm_control.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_storm_control_setup_access.value, 1)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_rs485_map_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_rs485_map.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_rs485_map_setup_access.value, 1)

            ### meter control setup ###
            self.connect_manager.setup_client.write_register(ecm.addr_meter_test_mode.value, 0)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_meter_demo_mode_timeout_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_meter_demo_mode_timeout.value, 60)
            self.connect_manager.setup_client.write_register(ecm.addr_meter_demo_mode_timeout_setup_access.value, 1)

            ### meter system setup / local time 제외 ###
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_description_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_installation_year.value, 1970)
            self.connect_manager.setup_client.write_register(ecm.addr_installation_month.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_installation_day.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_description_setup_access.value, 1)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_locale_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_timezone_offset.value, 540)
            self.connect_manager.setup_client.write_register(ecm.addr_temperature_unit.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_energy_unit.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_date_display_format.value, 0)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_summer_time_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_summer_time.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_start_month.value, 3)
            self.connect_manager.setup_client.write_register(ecm.addr_start_nth_day_of_month.value, 2)
            self.connect_manager.setup_client.write_register(ecm.addr_start_day_of_week.value, 0)
            self.connect_manager.setup_client.write_register(ecm.addr_start_minute.value, 120)
            self.connect_manager.setup_client.write_register(ecm.addr_end_month.value, 11)
            self.connect_manager.setup_client.write_register(ecm.addr_end_nth_day_of_month.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_end_day_of_week.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_end_minute.value, 120)
            self.connect_manager.setup_client.write_register(ecm.addr_summer_time_offset.value, 60)
            self.connect_manager.setup_client.write_register(ecm.addr_summer_time_setup_access.value, 1)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_ntp_setup_access.value, 1)
            # self.connect_manager.setup_client.write_register(ecm.addr_ntp_ip.value, 0A0A0A01) 값 변경 필요
            self.connect_manager.setup_client.write_register(ecm.addr_sync_mode.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_sync_period.value, 600)
            self.connect_manager.setup_client.write_register(ecm.addr_sync_max_drift.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_ntp_setup_access.value, 1)
            self.connect_manager.setup_client.read_holding_registers(ecm.addr_lcd_buzzer_setup_access.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_lcd_backlight_timeout.value, 300)
            self.connect_manager.setup_client.write_register(ecm.addr_lcd_backlight_low_level.value, 10)
            self.connect_manager.setup_client.write_register(ecm.addr_buzzer_for_button.value, 1)
            self.connect_manager.setup_client.write_register(ecm.addr_lcd_buzzer_setup_access.value, 1)
