...
def measurement_voltage_uitest(image):
...
# 색 감 비교 코드
def meas_vol_test():
    image = cv2.imread(voltage_image_path)
    ...
    if color_result1 < 5 and color_result2 < 5:
        ...
        ocr_error, right_error = measurement_voltage_uitest(cut_voltage_image)
...
def meas_cur_test():
    ...
    if color_result1 < 5 and color_result2 < 5:
        cut_current_image = cut_image(image, 0.25, 1, 0.2, 1)
        ocr_error, right_error = measurement_currnet_uitest(cut_current_image)
        if not ocr_error and not right_error:
            print("pass")
        else:
            print("Fail")
...
def meas_demand_test():
    ...
    if color_result1 < 5 and color_result2 < 5:
        cut_demand_image = cut_image(image, 0.25, 1, 0.2, 1)
        ocr_error, right_error = measurement_demand_uitest(cut_demand_image)
        if not ocr_error and not right_error:
            print("pass")
        else:
            print("Fail")
...
def meas_power_test():
    ...
    if color_result1 < 5 and color_result2 < 5:
        cut_power_image = cut_image(image, 0.25, 1, 0.2, 1)
        ocr_error, right_error = measurement_power_uitest(cut_power_image)
        if not ocr_error and not right_error:
            print("pass")
        else:
            print("Fail")
