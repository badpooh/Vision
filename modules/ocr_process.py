...
class ImgOCR:
    ...
    def img_ocr(self, loaded_image_path):
        ...
        png_files = [os.path.join(loaded_image_path, f) for f in os.listdir(
            loaded_image_path) if f.endswith('.png')]
        ...
        resized_image_1 = self.image[int(
            height*0.35):height, int(width*0.185):width]
        resized_image = cv2.resize(
            resized_image_1, None, None, 2, 2, cv2.INTER_CUBIC)

        reader = PaddleOCR.Reader(['en'], gpu=True)
