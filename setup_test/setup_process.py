...
class SetupTest(QObject):
	...
	def m_s_event_dip(self, base_save_path, search_pattern):
		...
		self.config_setup_action(
			...
			ref_value=list(ConfigROI.s_dip_trigger_2.value[1])[1],
			ref_select=1,
			modbus_ref=ConfigROI.s_dip_trigger_2.value[1]['Enable'],
		...
		self.config_setup_action(
			...
			except_addr=ConfigMap.addr_dip,
			access_address=ConfigMap.addr_dip_setup_access.value,
			ref_value=list(ConfigROI.s_dip_trigger_2.value[1])[0],
			ref_select=1,
			modbus_ref=ConfigROI.s_dip_trigger_2.value[1]['Disable'],
